...
public class SheduleJobReadPlatformServiceImpl implements
	...
	public Long getMessageId(String messageTemplateName) {
		...
		try {
			...
			return jdbcTemplate.queryForObject(sql, mapper,
					new Object[] { messageTemplateName });
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
	...
	@Override
	public List<ScheduleJobData> retrieveSheduleJobParameterDetails(
			String paramValue) {
		try {
	...
	public JobParameterData getJobParameters(String jobName) {
		try {
			List<JobParameters> jobParameters = this
					.retrieveJobParameters(jobName);// scheduledJobDetail.getDetails();
	...
	private static final class SheduleJobParametersMapper implements
			RowMapper<JobParameters> {
		public String sheduleLookupSchema() {
		...
		@Override
		public JobParameters mapRow(final ResultSet rs,
				@SuppressWarnings("unused") final int rowNum)
				throws SQLException {
			final Long id = rs.getLong("id");
			...
			return new JobParameters(id, jobId, paramName, paramType,
					defaultValue, paramValue, isDynamic, queryValue);
		}
	}
	...
	public List<ScheduleJobData> getJobQeryData() {
		try {
			...
			final SheduleJobMapper mapper = new SheduleJobMapper();
			final String sql = "select " + mapper.sheduleLookupSchema()
					+ "where sr.report_category='Scheduling Job'";
